...
const TABS = [
  ...
  { key: 'revision_requested', label: 'Revision', color: 'text-blue-400' },
]
...
function ProposalCard({ proposal, onAction }: { proposal: Proposal; onAction: () => void }) {
  ...
  async function submit() {
    if (!action) return
    if ((action === 'reject' || action === 'revise') && !feedback.trim()) return
    setLoading(true)
    try {
      if (action === 'approve') await proposalsApi.approve(proposal.id, feedback)
      if (action === 'reject') await proposalsApi.reject(proposal.id, feedback)
      if (action === 'revise') await proposalsApi.revise(proposal.id, feedback)
      onAction()
    } catch (e: any) {
      alert(e.message)
    } finally {
  ...
  return (
    <div className={`card flex flex-col gap-0 overflow-hidden transition-all ${expanded ? 'ring-1 ring-accent/30' : ''}`}>
      <div className="p-5">
        {/* Header */}
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <span className={`badge border shrink-0 ${STATUS_STYLE[proposal.status]}`}>
        ...
        <div className="border-t p-4 flex flex-col gap-3" style={{ borderColor: 'rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.02)' }}>
          {!action ? (
            <div className="flex gap-2">
              <button onClick={() => setAction('approve')} className="flex-1 flex items-center justify-center gap-2 rounded-lg py-2 text-xs font-medium text-emerald-400 transition-all hover:bg-emerald-500/15"
                      style={{ border: '1px solid rgba(52,211,153,0.2)' }}>
                <CheckCircle size={14} />
                Approve
              </button>
              <button onClick={() => setAction('revise')} className="flex-1 flex items-center justify-center gap-2 rounded-lg py-2 text-xs font-medium text-blue-400 transition-all hover:bg-blue-500/15"
                      style={{ border: '1px solid rgba(96,165,250,0.2)' }}>
                <RefreshCw size={14} />
                Request Revision
              </button>
              <button onClick={() => setAction('reject')} className="flex-1 flex items-center justify-center gap-2 rounded-lg py-2 text-xs font-medium text-red-400 transition-all hover:bg-red-500/15"
...
export default function Proposals() {
  ...
  async function load() {
    setLoading(true)
    try {
      setList(await proposalsApi.list(tab))
    } finally {
      setLoading(false)
    }
  }
        ...
        {TABS.map(({ key, label }) => (
          <button
            ...
            className={`rounded-lg px-4 py-1.5 text-xs font-medium transition-all ${
              tab === key ? 'bg-surface-card text-slate-100 shadow-sm' : 'text-slate-500 hover:text-slate-300'
            }`}
          >
            {label}
          </button>
