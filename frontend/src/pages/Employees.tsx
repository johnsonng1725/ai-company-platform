...
function EmployeeModal({ companyId, editing, onClose, onSaved }: {
  companyId: number; editing: Employee | null; onClose: () => void; onSaved: () => void
}) {
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              {['Anthropic', 'OpenAI'].map((provider) => (
                <div key={provider}>
                  <p className="mb-1 flex items-center gap-1.5 text-xs text-slate-600">{PROVIDER_ICON[provider]} {provider}</p>
                  <div className="flex flex-col gap-1">
                    {MODELS.filter((m) => m.provider === provider).map((m) => (
                      <button key={m.id} type="button" onClick={() => setModel(m.id)}
                        ...
                        <span className="flex items-center gap-3 text-slate-500"><span>{m.tier}</span><span className="font-mono tracking-wide">{m.cost}</span></span>
                      </button>
                    ))}
                  </div>
                </div>
...
export default function Employees() {
  ...
  async function load() {
    try { setList(await employeesApi.list(cid)) } finally { setLoading(false) }
  }
  useEffect(() => { load() }, [cid])
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {showModal && (
        <EmployeeModal companyId={cid} editing={editing} onClose={() => setShowModal(false)}
