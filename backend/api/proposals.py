...
def list_proposals(
    ...
    q = db.query(models.Proposal).filter(models.Proposal.company_id == company_id)
    if status:
        q = q.filter(models.Proposal.status == status)
    return [_enrich(p) for p in q.order_by(models.Proposal.created_at.desc()).all()]
...
def approve_proposal(
    ...
    proposal = _get_proposal(proposal_id, company_id, current_user, db)
    _assert_pending(proposal)
    proposal.status = "approved"
    proposal.ceo_feedback = data.feedback
    db.add(models.ActivityLog(
        company_id=company_id, employee_id=proposal.employee_id,
        level="success", message=f"✅ CEO approved: \"{proposal.title}\"",
...
def reject_proposal(
    ...
    db.add(models.ActivityLog(
        company_id=company_id, employee_id=proposal.employee_id,
        level="error", message=f"❌ CEO rejected: \"{proposal.title}\"",
        data={"proposal_id": proposal.id},
    ))
    db.commit()
    db.refresh(proposal)
...
def request_revision(
    ...
    db.add(models.ActivityLog(
        company_id=company_id, employee_id=proposal.employee_id,
        level="warning", message=f"🔄 CEO requested revision: \"{proposal.title}\"",
        data={"proposal_id": proposal.id},
    ))
    db.commit()
    db.refresh(proposal)
...
def _get_proposal(proposal_id: int, company_id: int, user: models.User, db: Session) -> models.Proposal:
    get_company(user, db, company_id)
    proposal = db.query(models.Proposal).filter(
        models.Proposal.id == proposal_id,
        models.Proposal.company_id == company_id,
    ).first()
    if not proposal:
